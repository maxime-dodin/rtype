...
void follow_system([[maybe_unused]] Registry &r, sparseArray<Follow> &followArr, sparseArray<Position> &posArr, sparseArray<Velocity> &velArr, MyClock &clock);
...
void EcsServer::addGameSystemsServer()
{
    ...
    _game.add_system_to_current_scene<Spawner, Position>(
            [&clock = getClock(), &loader = GetLoader()]
            (Registry &r, sparseArray<Spawner> &spawn, sparseArray<Position> &posArr)
            { spawner_system(r, clock, spawn, loader, posArr); }
    ...
    _game.add_system_to_current_scene<Follow, Position, Velocity>(
            ...
            { follow_system(r, follow, pos, vel, clock); }
        );
